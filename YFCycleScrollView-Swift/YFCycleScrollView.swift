...
class YFCycleScrollView: UIView {
    ...
    private func buildView() {
        ...
        pageControl.center = CGPoint.init(x: center.x, y: viewHeight - 15)
    ...
    private func startScroll() {
        if(!timer.isValid) {
            timer = Timer.scheduledTimer(timeInterval: 3, target: self, selector: #selector(leftScroll), userInfo: nil, repeats: true)
        }
    }
